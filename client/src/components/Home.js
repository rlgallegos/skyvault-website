...
function Home(){
    ...
    const navList = navItems.map(item => {
        return <Link key={item.id} to={item.id_name} smooth={true} duration={500} >
            <li 
                className="point lora text-2xl text-white mx-2 py-3 px-6 bg-transparent rounded border border-transparent 
    ...
    return (
        <div id="home" className="block bg-blue-100 relative">
            <div id="navbar" 
